...
\lstdefinelanguage{BCL}{
    keywords={PROC, END, WITH, DO, SET, GLOBAL, IF, THEN, ELSE,
              ...
              EVENT, CREATE, DELETE, TIMER, PROCESS, LOOP, STOP, INFO,
...
\subsection{Integración con Sistema de Eventos}

La extensión SOCKET está diseñada para integrarse perfectamente con el sistema de eventos de BCL v2.0. Los sockets retornan descriptores de archivo que pueden ser monitoreados con el comando \texttt{EVENT}.

\begin{notebox}
La integración con eventos permite crear servidores no bloqueantes que pueden manejar múltiples clientes simultáneamente sin hilos ni procesos adicionales.
\end{notebox}

\textbf{Ejemplo de uso con eventos:}
...
PROC AL_ACEPTAR WITH servidor_fd DO
    ...
    PUTS "Cliente conectado: $cliente"

    # Registrar callback para datos del cliente
    EVENT CREATE $cliente READABLE AL_DATOS_CLIENTE
END

# Callback para datos de cliente
PROC AL_DATOS_CLIENTE WITH cliente_fd DO
    GLOBAL clientes
    ...
    ELSE
        ...
        SOCKET SEND $cliente_fd "Echo: $datos"
    END
END

# Crear servidor y registrar evento
SET servidor [SOCKET SERVER 9999]
EVENT CREATE $servidor READABLE AL_ACEPTAR

PUTS "Servidor asíncrono escuchando en puerto 9999"
...
\textbf{Ventajas del enfoque asíncrono:}

\begin{itemize}
    \item Múltiples clientes sin hilos o procesos
    \item Bajo consumo de recursos
    \item Escalable hasta ~1000 conexiones con select()
    \item Compatible con timers y otros eventos
\end{itemize}

Para más información sobre el sistema de eventos, consulte:
\begin{itemize}
    \item \texttt{docs/EVENT\_SYSTEM.md} - Documentación completa del sistema de eventos
    \item Manual BCL, Capítulo 18 - Sistema de Eventos
\end{itemize}
