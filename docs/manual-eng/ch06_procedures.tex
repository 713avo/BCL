...
\textbf{Syntax without parameters:}
\begin{lstlisting}[language=BCL]
PROC name DO
  commands
  RETURN value
...
\begin{notebox}
When a procedure has no parameters, the \cmd{WITH} keyword can be omitted.
\end{notebox}

\begin{examplebox}[title=Procedure with Parameter]
\begin{lstlisting}[language=BCL]
...
\begin{examplebox}[title=Procedure without Parameters]
\begin{lstlisting}[language=BCL]
