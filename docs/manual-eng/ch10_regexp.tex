...
# Use NOCASE option for case-insensitive matching
...
\begin{tipbox}
Use the \texttt{NOCASE} option to make patterns match regardless of upper/lowercase. Place \texttt{NOCASE} after the text argument:
\begin{lstlisting}[language=BCL]
...
\texttt{ALL} & Count/match all occurrences (returns count) \\
\bottomrule
\end{tabular}
\end{table}
...
REGSUB pattern string replacement NOCASE       # Case-insensitive
REGSUB pattern string replacement ALL NOCASE   # Both options
\end{lstlisting}
